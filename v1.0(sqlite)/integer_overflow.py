...
def save_data(method, case, url, payloads, res):
    ...
    if res.headers["Content-Length"]:
        ...
        print "Content-Length : " + str(res.headers["Content-Length"])
        print "payload: " + str(payloads)
        f = open("integer_overflow_result.csv", "a+")
        
        # case2 and post
        if payloads:
            f.write(urlparse(url)[1] + "," + method + "," + case + ","  + payloads + ","  + str(res.headers["Content-Length"]) + "\n")
            f.close()
            
        # case1 and get, case2 and get
        else:
            f.write(urlparse(url)[1] + "," + method + "," + case + "," + url + ","  + str(res.headers["Content-Length"]) + "\n")
            f.close()
        ...
        try:
            ...
            if cookie is None:
                ...
                try:
                    ...
                    print '\ninteger overflow attack\'s stage 2 is suddenly done: ', end_time - start_time
                    print "*" * 120
...
def predict_integer_overflow_attack_time(attack_commands):
...
def main():
    ...
    try:
        ...
        cookie = None
    try:
        os.remove("integer_overflow_result.csv")
    except:
        pass
    ...
    attack_commands = ["-1", "0", "0x100", "0x1000", "0x3fffffff", "0x7ffffffe", "0x7fffffff", "0x80000000",\
                       "0xfffffffe", "0xffffffff", "0x10000", "0x100000", "128", "-127", "-32768", "32767",\
                       "65535", "2147483647", "-2147483648", "-9223372036854775808", "9223372036854775807"]
    links_to_visit_params = []
    
    predict_integer_overflow_attack_time(attack_commands)
...
#step1
    ...
    print '\ninteger overflow attack\'s stage 1 is done: ', end_time - start_time
    print "*" * 120
...
#         if there is no form, it return False 
            ...
            for result in results:
                ...
                if method == "GET":
                    ...
                    if url_with_params not in links_to_visit_params:
                        ...
                        integer_overflow_attack(payloads, "POST", action, "case2", attack_commands)
    ...
    print "*" * 120
    print "you send requests %s times" % (req_cnt - f_req_cnt)
    print '\ninteger overflow attack\'s stage 2 is done: ', end_time - start_time
    print "*" * 120
