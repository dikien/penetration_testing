...
def save_data(method, case, url, payloads, res):
    ...
    for error_string in error_strings:
        if res_contents.find(error_string) != -1:
            print error_string
            f = open("sqli_error_based.csv", "a+")
                
            if payloads:
                f.write(urlparse(url)[1] + "," + method + "," + case + ","  + str(payloads) + "," + "error based" + "\n")
                f.close()
                print url
...
def web_request(payloads, method, action, case):
    ...
    elif method == "GET" and case == "case20":
        try:
    ...
    elif method == "POST" and case == "case20":
...
def main():
    ...
    filename = "sql_error_strings"
    
    global error_strings
    error_strings = open(filename).read()
    error_strings = error_strings.split("\n")
    ...
    try:
        os.remove("sqli_error_based.csv")
    except:
        pass
