...
#     print res.url
    ...
    for attack_command in attack_commands:
        if res_content.find(attack_command) != -1:
...
def xss_attack(payloads, method, action, case):
    ...
    for url in urls:
        ...
        if urlparse(url)[4]:
            ...
            if url_with_params not in links_to_visit_params:
                ...
                xss_attack(payloads, "GET", url, "case1")
            ...
            for result in results:
                ...
                if method == "GET":
                    ...
                    if url_with_params not in links_to_visit_params:
                        ...
                        xss_attack(payloads, "GET", action, "case2")
                    ...
                    if url_with_params not in links_to_visit_params:
                        ...
                        xss_attack(payloads, "POST", action, "case3")
               
                        links_to_visit_params.append(url_with_params) 

def predict_xss_attack_time():
...
def main():
    ...
    usage        = '''./xss.py -t google'''
    
    parser = argparse.ArgumentParser(description = "xss for pen testing", \
                                     usage = usage)
    ...
    try:
        attack_commands = file("xss_query").read()
        attack_commands = attack_commands.split("\n")
        
    except:
        print "[+] there is no xss_query file"
        connection.close()
        sys.exit(0)
...
# case 2, 3
    ...
    print "*" * 120
    print '\nxss attack is done: ', end_time - start_time
    print "*" * 120
    connection.close()
